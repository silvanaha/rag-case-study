...
from src.rag.create_index_and_retrieve import get_bm25_retriever, get_ensemble_retriever, \
    get_faiss_vectorstore_retriever, run_query_on_index
...
class CreateIndexTestCase(TestCase):
    ...
    def test_create_bm25retriever(self):
        ...
        retriever = get_bm25_retriever(texts, source_id=111, num_results=2)
    ...
    def test_create_fass_vectorstore_retriever(self):
        texts = ["Das ist mein Sample Text", "Das ist mein zweiter inhaltsvoller Satz", "Rote Bete ist erstaunlich lecker"]
        retriever = get_faiss_vectorstore_retriever(texts, source_id=112, num_results=2)
    ...
    def test_get_ensemble_retriever(self):
        texts = ["Das ist mein Sample Text", "Das ist mein zweiter inhaltsvoller Satz", "Rote Bete ist erstaunlich lecker"]
        retriever = get_ensemble_retriever(texts, source_id=112, num_results=2)
    ...
    def test_query_retriever_with_ensemble(self):
        ...
        retriever = get_ensemble_retriever(texts, source_id=1, num_results=2)
        results = run_query_on_index(query, retriever)
    ...
    def test_query_retriever_with_faiss(self):
        texts = ["Das ist mein Sample Text", "Das ist mein zweiter inhaltsvoller Satz", "Rote Bete ist erstaunlich lecker"]
        query= "Gib mir Infos zu leckerem Inhalt"
        retriever = get_faiss_vectorstore_retriever(texts, source_id=1, num_results=2)
        results = run_query_on_index(query, retriever)
        assert_that(results).is_not_none()
        assert_that(results[0].page_content).is_equal_to(texts[1])
    ...
    def test_query_retriever_with_bm25(self):
        texts = ["Das ist mein Sample Text", "Das ist mein zweiter inhaltsvoller Satz", "Rote Bete ist erstaunlich lecker"]
        query= "Gib mir Infos zu leckerem Inhalt"
        retriever = get_bm25_retriever(texts, source_id=1, num_results=2)
        results = run_query_on_index(query, retriever)
        assert_that(results).is_not_none()
